...
### 技能模块图标统一（2026-04-16 完成，commit 9a941cc）

**原则：** 技能相关 UI 一律用 `Wand2`；积分/费用 badge 保留 `Zap`（语义不同）。

**替换范围：**
- `Sidebar.tsx`：技能 nav 图标 `Zap` → `Wand2`
- `SkillsPanel.tsx`：标题 Zap → Wand2，槽位/预览图标升级
- `SkillConfirmCard.tsx`：标题 Wand2，保存按钮 Bookmark，音频 Music2
- `SaveSkillModal.tsx`：标题 Zap → Wand2
- `KlingConfirmCard.tsx`：标题 Wand2，保存按钮 Bookmark（积分 Zap 保留）
- `ChatPanel.tsx`：活跃技能横幅 ⚡ → Wand2，保存技能弹窗标题 ⚡ → Wand2，SkillPickerModal 标题和列表图标 ⚡ → Wand2，专业模式发送按钮 Zap → PenLine

**不要动：** 积分消耗 badge（`<Zap size={9}/>`，位于工具栏 Kling 模式费用显示 + 三张确认卡积分行）——Zap 在这里代表"费用/积分"，语义正确。

---

### FailedFooter 接入 SingleCardGrid（2026-04-17 完成，commit a477cd5）

**问题：** `FailedFooter` 组件（`WorksPanel.tsx` line ~121）已定义但从未渲染，失败卡片的错误信息和重试按钮重复出现在多处。

**修复（`WorksPanel.tsx`）：**

- **缩略图叠层**：失败态改为仅显示居中 `AlertTriangle`（24px，`text-error/50`），不再叠加错误标题文字
- **Row 3（info 区第三行）**：移除失败态下的错误标题行，统一为日期行；失败时显示空字符串（`FailedFooter` 负责展示错误）
- **Action row Slot 2**：失败态不再显示重试按钮，改为 `invisible` 占位（与成功态无文件时一致）
- **`FailedFooter` 渲染**：在 action row 闭合 `</div>` 后、外层卡片 `</div>` 前，条件渲染 `<FailedFooter error={task.error} model={task.model} onRetry={() => onRetry(task.id)} />`

**`FailedFooter` 功能：**
- 始终可见的摘要行：`AlertTriangle` + 错误标题 + 重试按钮 + 详情展开按钮
- 展开后显示完整 `parseTaskError().message`
- 顶部 `border-t border-[rgba(255,255,255,0.06)]` 分隔线

*最后更新：2026-04-17（FailedFooter 接入 SingleCardGrid）*
